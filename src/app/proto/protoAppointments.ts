...
export const PROTO_APPOINTMENTS: ProtoAppointment[] = [
  {
    id: "1411527",
...
export const PROTO_APPOINTMENT_COUNT = PROTO_APPOINTMENTS.length;

/** Prefill for Site Pilot when Sarah can't find an appointment in the list. */
export const PROTO_APPOINTMENT_PILOT_QUERY =
...
export function getAppointmentRefundPilotQuery(appt: ProtoAppointment): string {
...
let selectedAppointmentId = PROTO_APPOINTMENTS[0]?.id ?? "";
...
export function setSelectedAppointmentId(id: string): void {
  if (PROTO_APPOINTMENTS.some((a) => a.id === id)) {
    selectedAppointmentId = id;
  }
}

export function getAppointment(id: string): ProtoAppointment | undefined {
  return PROTO_APPOINTMENTS.find((a) => a.id === id);
}
...
function syncCancellationReasonRow(card: HTMLElement, appt: ProtoAppointment): void {
...
function ensureHistoryCardCount(page: HTMLElement): () => void {
  ...
  const needed = PROTO_APPOINTMENTS.length;
...
function syncHistoryMeta(page: HTMLElement): void {
  const count = PROTO_APPOINTMENT_COUNT;
...
function syncDetailPricing(page: HTMLElement, appt: ProtoAppointment): void {
...
export function syncAppointmentHistory(
  ...
  cards.forEach((card, index) => {
    const appt = PROTO_APPOINTMENTS[index];
    if (!appt) return;
...
export function syncAppointmentDetails(
  ...
  const appt = getAppointment(id || selectedAppointmentId) ?? PROTO_APPOINTMENTS[0];
  if (!appt) return () => {};
