...
/** Filter DevTools console with: `[ProtoControlPanel]` */
export const PROTO_CONTROL_PANEL_LOG_PREFIX = "[ProtoControlPanel]";
...
export function logControlPanel(
  ...
  if (blocked) {
    console.warn(
      `${PROTO_CONTROL_PANEL_LOG_PREFIX} BLOCKED ${action}${
...
export function dumpControlPanelLog(): ControlPanelLogEntry[] {
  const copy = [...ring];
  console.group(`${PROTO_CONTROL_PANEL_LOG_PREFIX} dump (${copy.length} entries)`);
  console.table(
    copy.map((entry) => ({
      seq: entry.seq,
      at: entry.atIso,
