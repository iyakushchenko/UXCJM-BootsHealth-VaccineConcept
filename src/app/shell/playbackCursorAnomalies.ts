export const DEMO_CURSOR_SELECTOR = ".proto-chat-demo-cursor";
...
export function countDemoCursors(root: ParentNode = document): number {
  return root.querySelectorAll(DEMO_CURSOR_SELECTOR).length;
}

export function areAllDemoCursorsParked(root: ParentNode = document): boolean {
  const cursors = root.querySelectorAll<HTMLElement>(DEMO_CURSOR_SELECTOR);
