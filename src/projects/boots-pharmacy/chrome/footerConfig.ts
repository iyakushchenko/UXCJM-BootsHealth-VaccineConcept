import type { FooterProps } from "@/projects/boots-pharmacy/chrome/Footer";

/** Hide any native Figma footer before mounting Footer. */
export const FOOTER_HIDE_SELECTORS = [
...
/**
 * Footer variant per Frame219 DOM child index.
 * Matches each screen's native footer: full+columns (PLP-style), compact health (book steps), etc.
 */
export const FOOTER_BY_CHILD: Record<number, FooterProps> = {
...
/** Site Pilot (home + chat) — no Footer. */
export const SITE_PILOT_CHILD_INDICES = [10, 11] as const;

export const FOOTER_CHILD_INDICES = Object.keys(FOOTER_BY_CHILD)
  .map(Number)
  .sort((a, b) => a - b);
