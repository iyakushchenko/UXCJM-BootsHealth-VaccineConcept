...
} from "@/projects/boots-pharmacy/hub/HubImageLightbox";
import HubTabLink from "@/projects/boots-pharmacy/hub/HubTabLink";
import { studioScreenAtTab, studioTabToIndex } from "@/projects/boots-pharmacy/screens/screens";
...
function HubScreenLinks({
  ...
  return (
    <div className="proto-hub-page__screen-links">
      {tabs.map((tab) => {
        const guide = HUB_SCREEN_GUIDE.find((item) => item.tab === tab);
...
export default function HubPage({ onGoToTab }: Props) {
  const { title, lead, tourIntro, sections } = HUB_CONTENT;
          ...
          <div ref={bodyRef} className="proto-hub-page__body">
            ...
            <aside className="proto-hub-page__sidebar" aria-label="Page navigation">
              ...
              <button
                ...
                onClick={() => onGoToTab(studioTabToIndex(1))}
              >
                Open UX Concept
              </button>
