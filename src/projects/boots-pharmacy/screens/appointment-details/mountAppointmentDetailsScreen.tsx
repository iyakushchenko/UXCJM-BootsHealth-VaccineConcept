...
function pageEl(): HTMLElement | null {
  return document.querySelector(APPOINTMENT_DETAILS_SCREEN_SELECTOR);
}
...
function ensureHost(page: HTMLElement): HTMLElement {
  ...
  if (!host) {
    ...
    host.dataset.studioReactScreen = APPOINTMENT_DETAILS_REACT_SCREEN_ID;
...
export function mountAppointmentDetailsScreen(
  props: AppointmentDetailsScreenProps
): void {
  ...
  hideMakeChrome(page);
  page.dataset.studioReactScreen = APPOINTMENT_DETAILS_REACT_SCREEN_ID;
  const host = ensureHost(page);
  if (!root) root = createRoot(host);
  root.render(<AppointmentDetailsScreen {...props} />);
}
