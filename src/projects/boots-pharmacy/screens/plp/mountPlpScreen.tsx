...
function hideMakeChrome(page: HTMLElement): void {
  Array.from(page.children).forEach((node) => {
    ...
    if (
      ...
      node.dataset.studioReactScreen === PLP_REACT_SCREEN_ID
    ) {
      return;
    }
    node.style.display = "none";
    node.dataset.studioMakeRetired = PLP_REACT_SCREEN_ID;
  });
  page.dataset.studioReactScreen = PLP_REACT_SCREEN_ID;
}

function restoreMakeChrome(page: HTMLElement): void {
  page
    .querySelectorAll<HTMLElement>(
      `[data-studio-make-retired="${PLP_REACT_SCREEN_ID}"]`
    )
    .forEach((el) => {
