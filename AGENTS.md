...
0. **Stuck / unknown / same FAIL twice → READ, do not thrash (HARD).** Open [docs/product/AGENT_STUCK_ROUTER.md](docs/product/AGENT_STUCK_ROUTER.md) → **one** dig row → fix that cause → one prove. Forbidden: blind retries, inventing chrome, opening the whole docs jungle, burning tokens on repetitive loops. Proof helpers: [docs/shell/PROOF_ROUTER.md](docs/shell/PROOF_ROUTER.md).
...
1e. **PAGE CREATE INHERITANCE (HARD):** Before creating/migrating any page — [PAGE_CREATE_INHERITANCE.md](docs/product/PAGE_CREATE_INHERITANCE.md) P1–P6: UXDS inventory + similar UXDS frame + existing UXML kits/pages ([UXDS_MAP.md](docs/uxds/UXDS_MAP.md) · [REACT_KIT_MAP.md](docs/uxds/REACT_KIT_MAP.md)); inherit everything possible; interactive = shared kits (no dead headers); **project theme = brand/copy delta only** ([PROJECT_STYLEGUIDE.md](docs/product/PROJECT_STYLEGUIDE.md) · [CSS_BASE_THEME.md](docs/product/CSS_BASE_THEME.md)). Bea stamps inheritance table; Arch rejects build without it.  
1e2. **Reflex:** After each HARD-GREEN page, **Arch (Director)** runs a micro-retro into [TEAM_KNOWLEDGE.md](docs/product/TEAM_KNOWLEDGE.md) before opening the next migrated page — [TEAM.md](docs/product/TEAM.md) § Reflex · [TEAM_RETRO_2026-07-19_PLP.md](docs/product/TEAM_RETRO_2026-07-19_PLP.md).  
   → [PAGE_FINAL_PASS.md](docs/product/PAGE_FINAL_PASS.md) · [PAGE_CREATE_INHERITANCE.md](docs/product/PAGE_CREATE_INHERITANCE.md) · [NEXT_STEPS.md](docs/product/NEXT_STEPS.md) · [COMMAND_DOCTRINE.md](docs/product/COMMAND_DOCTRINE.md) §0 / §4
...
**Always (short):**  
0. Stuck? → [AGENT_STUCK_ROUTER.md](docs/product/AGENT_STUCK_ROUTER.md) (one dig) — then continue  
1. [COMMAND_DOCTRINE.md](docs/product/COMMAND_DOCTRINE.md) · [TEAM.md](docs/product/TEAM.md) · [TEAM_KNOWLEDGE.md](docs/product/TEAM_KNOWLEDGE.md) (your hat section) · [PAGE_FINAL_PASS.md](docs/product/PAGE_FINAL_PASS.md) · [PAGE_CREATE_INHERITANCE.md](docs/product/PAGE_CREATE_INHERITANCE.md) · [LESSONS_LEARNED.md](docs/product/LESSONS_LEARNED.md) · [NAMING.md](docs/product/NAMING.md) · [NEXT_STEPS.md](docs/product/NEXT_STEPS.md) · [POST_CHANGE_CHECKLIST.md](docs/product/POST_CHANGE_CHECKLIST.md)  
...
Pages / kits → [PAGE_BUILD_CONTRACT.md](docs/product/PAGE_BUILD_CONTRACT.md) · [PAGE_CREATE_INHERITANCE.md](docs/product/PAGE_CREATE_INHERITANCE.md) · [COMPONENT_LIBRARY.md](docs/product/COMPONENT_LIBRARY.md) · [INTERACTION_FIDELITY.md](docs/product/INTERACTION_FIDELITY.md) · [INTERACTION_INVENTORY.md](docs/product/INTERACTION_INVENTORY.md) (**HARD pre/post map for coarse concepts and React migrations**) · [docs/uxds/UXDS_MAP.md](docs/uxds/UXDS_MAP.md) (HARD exhaustive inventory) · [docs/uxds/REACT_KIT_MAP.md](docs/uxds/REACT_KIT_MAP.md) · [docs/uxds/README.md](docs/uxds/README.md)
...
X-Suite / Summarizer CJM export → [X_SUITE_INTEGRATION.md](docs/product/X_SUITE_INTEGRATION.md) · [DIRECTION_FROM_SUMMARIZER.md](docs/product/DIRECTION_FROM_SUMMARIZER.md) (analyze → pages → REC; ask for page prototypes)  
...
**Reset QA before every test (ALWAYS CLEAR = code law):** `requireFreshQaSession(title)` — `forceClear` + fresh `start` with **no skip flag**. Wired into `__studioArmRecCapture`, `__studioRunRecNewCjmProve`, `__studioRunFullPlayProve`. If arm is bypassed, `startRecording` also ensures QA overlay is active. Never reuse a dirty AGENT TESTING session. Smokes (`withMcpTestSession`) force-clear before arm. → [QA_LOGGING_AND_PLAYBACK_RECIPE.md](docs/shell/QA_LOGGING_AND_PLAYBACK_RECIPE.md) · [RECORDING.md](docs/shell/RECORDING.md)

**Standing prove shortcuts (HARD — PO can say these alone):** `uxml rec` · `uxml play` · `uxml play step` · `uxml play step r` → locked procedures in [UXML_COMMANDS.md](docs/shell/UXML_COMMANDS.md) (default = **current CJM** unless named). Always = QA overlay + Chrome DevTools MCP watched run on `:5173`. Do **not** invent a parallel path. → [PROOF_ROUTER.md](docs/shell/PROOF_ROUTER.md)

**REC robustness prove (HARD — NEW CJM only · = `uxml rec`):** `await window.__studioRunRecNewCjmProve?.({ experience })` — ALWAYS CLEAR → `__studioArmRecCapture` → **human-paced** path (`REC_USER_PACE_MS`: read / scroll-stop ≥2s / after-click) → **modal drain** if `&modal=choose-pharmacy` (pick pharmacy — never rush Continue) → Stop + Add as CJM → Play **that** journey. Returns `{ pass, journeyId, recLive, peak, errors }`. **FORBIDDEN** as REC prove: only playing built-in CJMs, reusing an old `rec-*`, or skipping open modals. Assert latch: `__studioAssertRecLive()`. → [RECORDING.md](docs/shell/RECORDING.md) · [CJM_RECORD_PLAY_EDIT.md](docs/shell/CJM_RECORD_PLAY_EDIT.md)

**Full continuous Play prove (HARD — universal · = `uxml play`):** call `await window.__studioRunFullPlayProve?.({ journeyId })` (current CJM) or `{ experience }` (alias `__protoRunFullPlayProve`; defaults via presets). ALWAYS CLEAR → arm → Play → peak → leave pause; keeps QA open for Save Log. Thin aliases: `__studioRunAgenticFullPlayProve` / `__studioRunTraditionalFullPlayProve` — **no** duplicated logic; **not** ad-hoc Play / **not** `*PlaySmoke` (tears down). Stepped = `uxml play step`; stepped + rewind = `uxml play step r`. → [UXML_COMMANDS.md](docs/shell/UXML_COMMANDS.md) · [QA_LOGGING_AND_PLAYBACK_RECIPE.md](docs/shell/QA_LOGGING_AND_PLAYBACK_RECIPE.md)
...
**Studio URL params:** `project` · `screen` · `persona` · `cjm=on|off` · `experience=agentic|traditional` · `modal` — **not** `mode=agentic-cjm` (legacy alias only). Example: `http://localhost:5173/?project=boots-pharmacy&screen=chat&persona=sarah-jenkins&cjm=on&experience=agentic`. **`modal=` is navigable state** — agents MUST read and interact (e.g. `modal=choose-pharmacy` after Book Step 1 Continue with no location); never ignore / rush past. → [docs/shell/URL.md](docs/shell/URL.md)
