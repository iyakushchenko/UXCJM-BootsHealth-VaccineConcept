...
**REC robustness prove (HARD — NEW CJM only):** `await window.__studioRunRecNewCjmProve?.({ experience: "traditional" })` — ALWAYS CLEAR → `__studioArmRecCapture` → **human-paced** path (`REC_USER_PACE_MS`: read / scroll-stop ≥2s / after-click) → **modal drain** if `&modal=choose-pharmacy` (pick pharmacy — never rush Continue) → Stop + Add as CJM → Play **that** journey. Returns `{ pass, journeyId, recLive, peak, errors }`. **FORBIDDEN** as REC prove: only playing built-in CJMs, reusing an old `rec-*`, or skipping open modals. Assert latch: `__studioAssertRecLive()`. → [RECORDING.md](docs/shell/RECORDING.md) · [CJM_RECORD_PLAY_EDIT.md](docs/shell/CJM_RECORD_PLAY_EDIT.md)
