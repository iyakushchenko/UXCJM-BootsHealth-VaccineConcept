...
**Truth in one line:** Step 1 body is a **proven hybrid pilot** (React host inside Make Frame child 7). Architecture layers are locked. LEGACY Make is still the house. Next page is allowed **only** if we refuse to grow that house.
...
| N4 | Proto header/footer vs Make absolute | Intentional Studio chrome; documented in design delta. |
...
3. **Dual-system bugs** — Make date-cell scripts + React calendar both live → CJM/playback ghost clicks.  
...
6. **Funnel cliff** — Continue already lands on **Make** Step 2; a sloppy React Step 2 that doesn’t match wire state (slot, location, vaccine) breaks Traditional CJM.
...
- [ ] Next page ships without a separate FE Nazi QA audit file under `docs/product/audits/` ending in **PROVEN**
...
5. **Keep wire state contract** (chosen location, vaccine, recipient, booster, booking slot) across React/Make boundary.  
