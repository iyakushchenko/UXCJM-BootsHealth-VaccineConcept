...
| **Chat bubble appear (north star)** | Motion-only `CHAT_PULL_UP` (opacity + y, `STUDIO_ENTER_MS`, `MOTION_EASE_IN_OUT`) | **One** arrive language for progressive q/r **and** composer-send thinking→reply. Thinking exit = opacity-only at **same** duration. No CSS enter/transition on `.chat__bubble`. Co-travel camera (no appear-then-yank). Industry bar: message arrives once, continuous ease, in-place thinking→reply. Rails: [CHAT_PAGE_RAILS.md](../projects/boots-pharmacy/features/CHAT_PAGE_RAILS.md). |
