...
| PP-13 | QA-tool trust (dual-role self-test) | Overlay is **load-bearing** for agent mid-flight work. Observe↔agent handoff, Alarm escalate, ask/PENDING, refresh hydrate, and REC XOR must stay trustworthy — agents **must** re-run self-test after overlay changes. | IN PROGRESS | Marathon 2026-07-20: observe Alarm escalate fixed; SELF_TEST.md + `__studioRunQaSelfTestSmoke` · Quinn/Finn |
