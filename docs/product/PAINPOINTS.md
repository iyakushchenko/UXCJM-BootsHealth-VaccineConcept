...
| PP-13 | QA-tool trust (dual-role self-test) | Overlay is **load-bearing** for agent mid-flight work. Observe↔agent handoff, Alarm escalate, ask/PENDING, refresh hydrate, and REC XOR must stay trustworthy — agents **must** re-run self-test after overlay changes. | IN PROGRESS | Dual-use 2026-07-20: Save Log=current session; REC preserves observe; Session shows Screen/Beat; dump embeds `mcp`. Residual: custom CJM beat vs orchestra 1/N; click selector depth. SELF_TEST + README · Quinn/Finn |
