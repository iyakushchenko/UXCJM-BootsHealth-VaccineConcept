...
| PP-13 | QA-tool trust (dual-role self-test) | Overlay is **load-bearing** for agent mid-flight work. Observe↔agent handoff, Alarm escalate, ask/PENDING, refresh hydrate, and REC XOR must stay trustworthy — agents **must** re-run self-test after overlay changes. | IN PROGRESS | v0.0.83+: Beat honesty for rec-*; click selectors in dump; duration clamp; 10px CONTROL border; observe cursor follow; click 1:1; Save Log snapshot; RESULT finale. Sole-brain closer, not absolute YES. SELF_TEST marathon · Quinn/Finn |
