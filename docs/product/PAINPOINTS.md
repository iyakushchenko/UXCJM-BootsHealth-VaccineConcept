...
| PP-08 | Insufficient logging | Not enough mid-flight signal; identical helper spam ≠ useful log. | WATCH | v0.0.91: all QA toolbar actions logged (Save Log/Pause/Close/Reset/forceClear) |
...
| PP-15 | Diag popup vs QA dump | PO does not need the diagnostic modal if agents get the data. Prefer dump/ring ingest; keep Alarm; optional consume dismiss. | WATCH | v0.0.91: auto-dismiss on session end/self-test/finale + suppress re-open · leftover modal prove PASS |
| PP-16 | QA does not listen | PO furious — tool ignores Pause / Message / PENDING / diag / diode; agents keep acting as if nothing happened. | WATCH | v0.0.90 trust-breakers + v0.0.91 action sitrep; M3 Pause halt proved |
| PP-17 | Pause must halt Play | Pause in QA (manual/agent/observe) must stop playback/progress immediately — was no-op for non-agent. | COMPLETE | v0.0.90 + campaign M3: Play→Pause → `isPlaying` false · latch `QA_PAUSE_HALT` |
| PP-18 | Message must be honored | Send (esp. PENDING / mid-Play) must pause progress, latch user message, require agent read→investigate→fix→reply before proceed. | COMPLETE | v0.0.90 `USER_MESSAGE_RECEIVED` · README procedure |
| PP-19 | PENDING + typing race | Focusing/typing Message while PENDING must signal wait (extend timeout); draft must survive refresh. | COMPLETE | v0.0.90 typing extend + draft |
| PP-20 | Message autofocus + draft | Message field must autofocus on QA open + after refresh restore; draft in sessionStorage. | COMPLETE | v0.0.90 |
| PP-21 | QA unaware of playback diag | When diagnostic modal open, stop/ignore Play; ingest to dump; color-coded log; halt until Ack/consume. | WATCH | v0.0.90 latch + v0.0.91 auto-dismiss leak fix |
| PP-22 | Control-room diode / Alarm red ignored | QA must notice + log sitrep; not act like nothing happening. | WATCH | diagnostic-open → control-room Alarm-red log + latch |
| PP-23 | MCP phase changes invisible | CONNECTING→CONTROL→OBSERVE→PENDING→ERROR must appear in QA timeline for sitrep. | COMPLETE | v0.0.90 `MCP · prev → PHASE` rows |
...
| PP-26 | QA action blindness | PO cannot see Save Log / export / toolbar lifecycle in timeline — sitrep incomplete. | COMPLETE | v0.0.91 `Save Log ·` / `QA · Pause` / `Close ·` / `forceClear ·` rows |
| PP-27 | Stale playback diagnostic leak | Modal left open after self-test/SF/Play jobs — blocks further work. | COMPLETE | v0.0.91 `__studioClearStalePlaybackDiagnostic` + forceClear/finale/self-test; leftover prove PASS |
