...
| 2026-07-19 | **PO rage list hard-guardrails:** (1) ONE text-link pattern — footer-like (no underline rest / underline hover); `.uxds-link` ≡ `.proto-link`; `check:links` in `npm test`. (2) Component library plan — migrate pages into real React kits, not Make slop (COMPONENT_LIBRARY.md). (3) REC disabled/forced off during AIR (same lock as transport/CJM). (4) Blast-radius self-check after any UI change (doctrine + director rule). (5) Node 22 everywhere. (6) GitHub Languages ≠ React (library in .tsx — documented). (7) Lean Actions budget (CI_ACTIONS_BUDGET.md). |
