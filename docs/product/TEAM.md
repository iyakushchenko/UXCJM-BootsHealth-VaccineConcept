...
- **PO mid-flight signals (HARD — Auto-Rule R15):** during MCP smoke the agent owns the window. Poll `__studioPeekPoSignal` / `__studioConsumePoSignal` (or peek `__studioAgentTestingTakeover`) **each beat**. Alarm = sequence / expected-steps mismatch — **live consume first**, dump secondary. On alarm: pause/branch investigate (e.g. progressive disclosure). → [PLAYBACK_DIAG.md](../shell/PLAYBACK_DIAG.md) · [STUDIO_AUTO_RULES.md](./STUDIO_AUTO_RULES.md) R15  
