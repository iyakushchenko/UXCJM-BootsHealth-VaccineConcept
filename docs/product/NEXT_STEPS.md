...
6. [~] **Chat React migration** — `CHAT_REACT_MOUNT_ENABLED=true` (Make child 10 retired via `data-studio-make-retired`). Shared `SitePilotComposer`. Register [CHAT_MAKE_PARITY_REGISTER.md](../projects/boots-pharmacy/features/CHAT_MAKE_PARITY_REGISTER.md) · brief [CHAT_REACT.md](../projects/boots-pharmacy/features/CHAT_REACT.md) · **NOT hard-green / NOT PROVEN** (no false stamp). Site Pilot HARD-GREEN unchanged. Residuals: Uma §0a PARTIAL (composer identity PASS; §0b + feedback open); Quinn expanded chat recipe (recipe PASS ≠ Final Pass); traditional `pdp-book-now` Make-first-match fix shipped.
