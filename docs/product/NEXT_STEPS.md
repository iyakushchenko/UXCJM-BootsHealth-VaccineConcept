...
7c. [~] **Header re-author (Make clone → hand-built UXDS React)** — 2026-07-23, gate-green / MCP re-prove owed. `setupHeader` no longer does `sourceHeader.cloneNode(true)` anywhere — `headerMount.tsx` now `createRoot`+`flushSync`-renders a hand-authored `<Header>` (`src/projects/boots-pharmacy/chrome/Header.tsx` + `headerContent.ts`, same extraction shape as `Footer.tsx`/`footerContent.ts`: nav items, logo/chevron SVG paths, aux Search/Login glyphs, 5-item logged-in + 4-item guest account-flyout menu — all copied 1:1 from the prior hand-rolled `innerHTML` strings, none invented). `attachHealthServicesMegaMenu`/`attachFullScreenSearch` (unchanged kits) now attach to the React-rendered header DOM instead of the Make clone. Header shell + flyout CSS split to colocated `chrome/header.css` (file-hygiene ceiling — same pattern as `mega-menu-flyout.css`). `npm test` 162/162 files (1008/1008 tests) green, `npm run build` green. **Owed before PAGE FINAL PASS HARD-GREEN stamp:** live MCP re-prove (header renders on ≥2 screens, Health Services mega menu + Full Screen Search open/position correctly from the new header, avatar/login flyout works, no visual regression vs. old Make header) — no browser/MCP transport was available in this session; do **not** self-declare HARD-GREEN without it. `frame/index.tsx` Make header markup is now unreferenced but the file itself is untouched (Phase E). Retro: [TEAM_RETRO_2026-07-22_HISTORY_DETAILS.md](./TEAM_RETRO_2026-07-22_HISTORY_DETAILS.md).
8. [x] **Delete Book Step 1–3 Make children** — 2026-07-23. `retireMakeUnderPage` gained a `permanent: true` mode (no park entry, no restore path); Book Step 1/2/3 mounts now call it instead of park+restore, so Make header/breadcrumbs/body/footer nodes are deleted outright on first mount and can never come back. Also swept the 2 narrowly-dead CSS blocks (`.proto-confirm-open-appointment*`, `.proto-page-booster-checkbox*`) whose only possible producer was already-unreachable Make-path wire code and whose only target was the now-deleted `[data-name="body"]` subtree. `frame/index.tsx` JSX itself (and the larger "Make path only" wire dead-code family in `BootsPharmacyProjectView.tsx`) is untouched — out of scope (Header = Phase D lane 7c; LEGACY wire/CSS shrink = board #9). `npm test` 162/162 files green, `npm run build` green.
9. [~] **LEGACY retirement (by screen)** — shrink Make wire + `globals-screens` as React pages land. Concept `.proto-*` classes retire with their screen. **Scoping note (2026-07-23, from Phase B):** `BootsPharmacyProjectView.tsx` has ~10 `useEffect` blocks (~1500+ lines) commented "Make path only" and gated solely by `isBookStep{1,2,3}ReactMounted() → return` — since Book Step 1–3 mount unconditionally (no flag), these guards are always true and the effect bodies are permanently unreachable today. Prime first cut for this item. **Phase C — mechanical CSS shrink (2026-07-23, done for this pass):** removed 114 dead lines from `globals-screens.css` (3004→2891) — the `.proto-chosen-tile` / `.proto-chosen-store-card` / `.proto-chosen-checkbox` / `.proto-chosen-map` (bare, viewport, surface, dragging) / `.proto-page-map-pin` rule family under `.proto-chosen-slot` (only producer was `setupChosenPageMap` in `locationsMap.ts`, called solely from the dead Book Step 1 "chosen location" effect) plus the Book Step 1 (`nth-child(7)`) `.utility / cursor` hide rule (Make body there is now permanently deleted, no React producer). **Grep-proven dead** — zero remaining references outside the dead effect / the removed CSS. **Explicitly left in place (live, not dead):** `.proto-chosen-slot` base rule + `.proto-chosen-map-bg` (both still rendered by `BookStep1LocationScreen.tsx`), `.proto-confirm-advantage*` (rendered by `BookStep3ConfirmationScreen.tsx`), `component.book.appointment.progress` / `data-studio-book-step-back` rules + calendar-cell `data-studio-cal-*` rules (all still emitted by the live `BookAppointmentProgress` UXDS kit / `BookStep2DateTimeScreen.tsx` inside the same `nth-child(N)` host) — same family Phase B already verified live; not re-touched. The ~1500-line dead-`useEffect` JS family in `BootsPharmacyProjectView.tsx` itself is **still untouched** (not required to prove the CSS deletions above) — remains this item's next cut. Live `uxml play`/visual re-prove of Book Step 1–3 owed (no browser/MCP transport available this pass — static grep-proof only, per Phase B's own residual pattern).
