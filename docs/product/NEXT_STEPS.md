...
7c. [ ] **Header re-author (Make clone → hand-built UXDS React)** — `src/projects/boots-pharmacy/chrome/headerMount.tsx` `setupHeader` still does `sourceHeader.cloneNode(true)` on a **live** Make-authored DOM node (`[data-name="boots-pharmacy.module.header"]`) from `src/projects/boots-pharmacy/frame/index.tsx` at runtime, with **no** extracted content data file (contrast `src/projects/boots-pharmacy/chrome/footerContent.ts` + `Footer.tsx`, already fully hand-authored and Make-free). Own **PAGE FINAL PASS–gated** lane (Uma fidelity + Quinn MCP matrix like a migrated page) before `frame/index.tsx` (~13,642 lines) can be deleted. Retro: [TEAM_RETRO_2026-07-22_HISTORY_DETAILS.md](./TEAM_RETRO_2026-07-22_HISTORY_DETAILS.md).
