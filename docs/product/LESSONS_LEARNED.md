...
- **Symptom / risk class:** `headerMount.tsx`'s `sourceHeader.cloneNode(true)` was replaced with a `createRoot`-mounted hand-authored `<Header>` (mirroring `Footer.tsx`/`footerContent.ts`). While consolidating the avatar-dot/badge-highlight side effects that used to live inside `setupHeader()`'s `if (label) { … }` guard, `document.addEventListener(SAVED_LOCATIONS_CHANGE_EVENT, onSavedLocationsChange)` was moved to unconditional **module top-level** (alongside the always-safe `subscribeStudioLoggedIn(...)` call already there). `npm run build` (esbuild, no type/runtime check of module side effects) stayed green; only `npm run test` (real `vitest run` execution) surfaced `ReferenceError: document is not defined` in a Node-environment test (`agenticRetreatContract.test.ts`) that transitively imports `headerMount.tsx` via `playback/traditional.ts` without a DOM (jsdom/happy-dom) environment.
- **Root cause:** the original code guarded every `document.*` / DOM-touching side effect behind `setupHeader()`'s own call (only invoked by DOM-environment code paths); moving one such call to unconditional module scope makes it run at **import time**, in whatever environment happens to import the module first — including plain-Node test environments that never call `setupHeader()`.
- **Rule of thumb:** any singleton chrome-mount module (`*Mount.tsx` style, header/footer/nav) may keep `subscribeStudioLoggedIn`-style pure-JS-store subscriptions at module top-level (safe, no DOM access), but any `document.addEventListener` / `document.querySelector` / DOM-construction call must stay **lazy** — guarded inside the mount function itself, with a one-time-bound flag (e.g. `if (!boundOnce) { boundOnce = true; document.addEventListener(...); }`) so it only ever runs once real DOM access is guaranteed.
- **Gate:** fixed by moving the listener bind into `setupHeader()` behind a `savedLocationsListenerBound` flag. `npm test` 162/162 files (1008/1008 tests) green; `npm run build` green. Also split the new header shell/flyout CSS into a colocated `src/projects/boots-pharmacy/chrome/header.css` (same pattern as `src/uxds/interactions/mega-menu-flyout.css`) after it pushed `globals-chrome.css` over the `check:hygiene` allowlist ceiling — prefer domain split over bumping the ceiling, per `HYGIENE.md`.

## 2026-07-23 — Header re-author (Phase D) follow-up: `flushSync` inside a `useEffect` = illegal nested render, caught by live MCP console, not by tests/build

- **Symptom / risk class:** live Chrome DevTools MCP re-proof on `?screen=plp&...&experience=traditional` surfaced `Warning: flushSync was called from inside a lifecycle method. React cannot flush when React is already rendering.` pointing at `BootsPharmacyProjectView.tsx`'s mount `useEffect`. Visually the header still rendered correctly (screenshot-confirmed) and the mega-menu/FSC kits still attached in that run — `npm test` / `npm run build` had already been green, because neither exercises a real browser console. **Tests + build green ≠ no console warnings; a live MCP pass is a distinct, necessary gate for this class of bug.**
- **Root cause:** `headerMount.tsx`'s `setupHeader()` called `flushSync(() => renderHeader())` to force `headerMount.firstElementChild` to exist synchronously (needed before `attachHealthServicesMegaMenu`/`attachFullScreenSearch` do direct DOM reads/appends on it) — but `setupHeader()` itself is invoked from inside a host `useEffect`, i.e. while React is still mid-flush of that commit's passive effects. `flushSync` re-entering the renderer while it's already rendering is a hard React rule violation, not just a style nit.
- **Fix:** wrap the `flushSync(() => renderHeader())` call (+ the two `attach*` calls that depend on its synchronous result) in `queueMicrotask(...)`, deferring past the current passive-effect-flush call stack — by microtask time React is no longer "already rendering", so the same `flushSync` call is legal again, and it still lands before the browser paints (no visible flash). The dependent host-view height-measure `useEffect` (reads `.proto-header-mount` height for `--sticky-top`) was updated to tolerate the header's content landing a microtask later on the very first run — same existing `runMaAvatars`-style rAF/timeout retry idiom already in that effect, extended to also re-measure height, rather than inventing a new mechanism.
- **Rule of thumb:** any `flushSync` call must be audited for **who calls the function that calls it**, not just the immediate call site — a `flushSync` deep inside a mount/setup helper is safe when called from a click handler but illegal when that same helper is invoked from a `useEffect`/`useLayoutEffect` of another component's render. When the synchronous-DOM-availability guarantee is still needed after deferring, prefer `queueMicrotask` (still pre-paint) over `setTimeout(0)` (a macrotask, can visibly flash).
- **Gate:** `npm test` 162/162 files (1008/1008 tests) green; `npm run build` green. Live MCP re-proof owed as a follow-up by the parent agent (not run in this pass — no browser available here) to confirm the console warning is gone.
...
## 2026-07-23 — Chat bubble motion self-test only ever gated the isolated transform, never the composited on-screen (layout) position PO actually sees (PP-14)

- **Symptom / risk class:** PO reported chat progressive/thinking→reply bubble pull-up "still choppy" **after** a self-test suite reporting 8/8 PASS across multiple prior agent "fixes." Board explicitly distrusted the self-test's green: "layout ΔY during camera co-travel was excluded from the hard gate." This is the exact "self-test green, PO says broken" failure class — a coarse pass/fail unit-style assertion missed a real motion-quality signal that a frame-by-frame trace would catch.
- **Root cause (confirmed by code read, not guessed):** `chatBubbleMotionSelfTest.ts`'s `analyzeChatBubbleMotionSamples` ran a **monotonic-descent shape check** (`isContinuousEase`, forbidding a >2px upward reversal after descent begins) only against the Motion **transform** `y` (`ySeries()` / `continuousY`). The **layout** position (`bubble.layoutY` = `getBoundingClientRect().top` — the actual on-screen pixel, i.e. transform **plus** the independently-driven camera co-travel scroll) was checked only by `maxAbsDeltaY <= 10`, a *magnitude* cap on frame-to-frame acceleration — never a *sign-consistency* (jitter) check. Two independently-eased animations (framer-motion transform tween + a separate scroll tween) can each individually be smooth yet drift out of phase frame-to-frame, producing a composited position that oscillates (e.g. -2,+3,-4,+2 px) entirely inside the 10px magnitude band. That reads self-test-green while a human eye at 60fps perceives it as choppy, because a magnitude cap tolerates direction reversals a monotonicity check would catch.
- **Live re-prove found the reported jank NOT currently reproducible** — Step forward through the full `q0→r3` scripted thread on `:5173` (`cjm=on`), cross-checked with (a) the internal diag bundle and (b) an **independent** raw `requestAnimationFrame` sampler reading `getBoundingClientRect().top` every real frame (bypassing the diag ring's own perf decimation — it keeps only every 4th "boring" frame via `BUBBLE_FRAME_SAMPLE_EVERY`), both showed strictly monotonic transform **and** layout descent, `jumps=0 chops=0`, across all 7 sampled bubbles. Most likely already fixed by the prior `7e0cce7` co-travel re-anchor ship; PO's report likely predates that landing fully, or was perf-variance on their machine. **Not reproducible ≠ close the loop without hardening the gate** — the structural loophole above is real and independently provable regardless of current live state.
- **Fix:** added `layoutYSeries()` + `continuousLayoutY` to the self-test, gated into each bubble's `ok` and given its own named `layout-continuous` check (not buried inside the generic `bubbles-ok` aggregate, so a future regression shows up by name in the report). Proven with a unit test that constructs a synthetic in-band layoutY wobble (transform perfectly smooth, layout oscillating within the 10px cap) and confirms the **old** gate would have passed it while the **new** gate correctly fails it — then confirmed **non-regressive** by running the new check against the live production bundle (`layout-continuous: 7/7 ok`).
- **Rule of thumb (generalizes beyond chat):** when a motion/scroll self-test combines two independently-animated signals (e.g. a CSS/Motion transform **and** a separately-driven scroll/camera position) into one visual result, a hard gate that checks each signal's magnitude in isolation is not equivalent to checking the **composited, on-screen** result for smoothness. Always add (or verify) a monotonicity/sign-consistency check against the *combined* screen-space measurement (here: `getBoundingClientRect()`), not just against each contributing animation's own internal delta. A magnitude-only cap is a necessary but insufficient smoothness gate whenever more than one animation driver can touch the same visual property.
- **Gate:** `src/app/shell/agent-testing/__tests__/chatBubbleMotionSelfTest.test.ts` — new test `FAIL on layoutY co-travel wobble that stays under the |ΔΔY|<=10 magnitude cap (PP-14)`; existing 5 tests unaffected (fixtures without `layoutY` samples now correctly no-op the new check rather than false-failing on absent data). `npm test` 1009/1009; `npm run build` green. Live MCP evidence + full writeup: [PAINPOINTS.md](./PAINPOINTS.md) PP-14 evidence section.
