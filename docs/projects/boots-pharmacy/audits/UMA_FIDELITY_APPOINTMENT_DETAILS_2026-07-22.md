...
**Surface:** Boots Pharmacy Appointment Details (`screenId: appointment-details`, Frame child **1**, Make Details / Right Column)  
**Date:** 2026-07-22  
**Owner:** Uma (UI/UX)  
**Status:** **PROVEN** — live MCP re-measure vs densify Make live look (20/20) + register intent  
**React target:** `screens/appointment-details/*` — **live** at `http://localhost:5173/?project=boots-pharmacy&screen=appointment-details`  
**Make truth:** Frame child **1** · densify live pad/gap **20/20** · info pad **20** · CTA gap **12** · title **20/28** · wire `syncAppointmentDetails` / status tones · `data/appointments.ts` `APPOINTMENTS` + selected id · **Date and Time** from SSoT `appointmentDate` (closes Make wire miss)  
**Register:** [APPOINTMENT_DETAILS_MAKE_PARITY_REGISTER.md](../features/APPOINTMENT_DETAILS_MAKE_PARITY_REGISTER.md) · brief [APPOINTMENT_DETAILS_REACT.md](../features/APPOINTMENT_DETAILS_REACT.md)  
...
**Not claimed:** next erase-Make page beyond History/Details close (Book Make delete still residual)
...
- **Make densify truth (register L9/L11 + LESSON 2026-07-22):** live Make Details was densified **20/20** pad/gap (not History Frame 32/56). React page CSS implements densify intent; globals densify gated `:not([data-studio-react-screen])` on child **1**.
...
- **Cancel hover invent:** MCP hover Cancel → color stays `rgb(92,92,92)` · stroke `rgb(175,204,202)` · **`invent: false`** · no React `#c96b6b` rule (Make densify Cancel hover remains gated off React host).
...
- **Date and Time:** label present; value SSoT e.g. `#1411527` → `Thursday, 26th June 2026, 10:30` (not stale Make hire date).
...
- **Make park:** Make body detached under React host (`data-studio-react-screen` on page + main); retired nodes parked off-DOM (stamp may be absent from live query — expected for `retireMakeUnderPage`).
- **Quinn:** probe audit **pending** this wave.

---

## Layout bands — Make Details inventory
...
| L6 | Nav menu items + active grey | **PASS** (P1 visual) | active Appointment history `#f5f5f5` (Make truth) |
...
| L12–L19 | Info rows (number → Location) | **PASS** | labels match Details Make (Vaccine Service · Phone Number) |
...
| L26 | Vaccinations accordion body | **N/A** | Make has none — React invents none |
...
| # | Stamp | Notes |
|---|-------|-------|
| LE1–LE3 | **N/A** | Make has no page loader / empty / updating — React invents none |
...
| Element | Make densify / register | React (MCP) | Status |
|---------|-------------------------|-------------|--------|
...
- Account nav item routing not wired (P1 visual) — active stays Appointment history per Make.
- Breadcrumb Home visual only — no invent Home route.
- Card width fluid in content column vs Make fixed 976 at full frame — acceptable under 1312 shell + sidebar.
- Make densify Cancel `#c96b6b` still exists for **Make-only** path (gated); React deliberately under-matches.
...
**Knowledge used:** TEAM_KNOWLEDGE Uma · UMA_FIDELITY_NOTES §0/§0a · VISUAL_FIDELITY §1.2 · LESSONS densify 2026-07-22 + invent-vs-Make · register densify 20/20 Details truth · no invent Cancel / accordion / Shipping.

---

## Related

- [APPOINTMENT_DETAILS_MAKE_PARITY_REGISTER.md](../features/APPOINTMENT_DETAILS_MAKE_PARITY_REGISTER.md)  
