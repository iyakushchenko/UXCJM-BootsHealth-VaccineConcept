# Appointment Details Make → React parity register
...
**Register interpretation:** Make columns = Frame/wire inventory. React columns = mount + Final Pass evidence (2026-07-22).  
**React target:** `src/projects/boots-pharmacy/screens/appointment-details/*` — **live** (`mountAppointmentDetailsScreen`; Make retired)  
...
**Project wire truth (HARD):** Appointment copy over Make order chrome is **boots-pharmacy project content** — not UXDS domain remaps. Keep Make `data-name`s where playback/continuity needs them (`Info Blocks / Order Summary/NO`, card host, etc.).
...
| L4 | P0 | **Body fill** (`data-name="body"`) | **Present** | **Missing** | Make body + densify column rules |
...
| L15 | P0 | **Row — Vaccine Service** | **Present** | **Missing** | Make label (≠ History “Vaccine”); value SSoT |
| L16 | P0 | **Row — Recipient** | **Present** | **Missing** | SSoT |
| L17 | P0 | **Row — Email** | **Present** | **Missing** | SSoT |
| L18 | P0 | **Row — Phone Number** | **Present** | **Missing** | Make label; value SSoT |
| L19 | P0 | **Row — Location** | **Present** | **Missing** | SSoT |
| L20 | P0 | **Row — Date and Time** | **Partial** | **Missing** | Make label Present; wire miss on value — React binds `appointmentDate` |
...
| L26 | P1 | **Vaccinations accordion body** | **N/A** | **N/A** | Make has no body — no invent |
...
| L34 | P1 | **Contact Information / Payment Details** static | **Present** | **Missing** | Make static; do not invent new identity |
...
| LE3 | P0 | Updating / refreshing overlay | **N/A** | **N/A** | Make has none — **no invent** |
...
| I13 | P0 | **Densify vs React page CSS** | **Present** (Make) | **Missing** | Gate child-1 densify like History |
...
| W2 | P0 | Make retired / parked | — | **Missing** | `retireMakeUnderPage` |
...
| W6 | P0 | Densify `:not([data-studio-react-screen])` on child 1 | **Partial** | **Missing** | History gated; Details still densifies Make — extend on mount |
...
| C4 | P0 | Crumb back → History React | **Present** | **Missing** | Must not hit Make History |
...
| B3 | Delete Book Step 1–3 Make children (board #8) after CJM/playback green |
