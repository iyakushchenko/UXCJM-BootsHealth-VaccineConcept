...
- CJM picker metadata is diagnostic truth: warning triangle = potential compatibility/provenance issue; hover explains it; click opens exact diagnostics and can copy the complete JSON for agent handoff. Patch-version drift alone is an amber **re-test required** state, never proof of incompatibility. Only structural contract/target failures block playback. Successful autonomous playback stores a contract proof receipt separately from immutable recording provenance, so unrelated releases do not invalidate proven CJMs.
